class SessionsController < ApplicationController
	...
	def create
	   ...
	   if user && user.authenticate(params[:session][:password]) #if the user is authenticated – we used this in part 2 as well
		    sign_in user #use the helper to log the user in by saving its id to the session hash
	...
	def destroy
		sign_out   #log the user out, this is again defined in the sessionsHelper
		redirect_to home_index_path #redirect the user to the homepage
	end
end
